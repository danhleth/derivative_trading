...
class BacktestDerivatives:
    ...
    def _report(self):
        print(f"Backtest report {self.type_df.capitalize()} set")
    ...
    def run_from_df(self, df, type_df='train'):
        self.start_date = df['start_time'].values[0]
        self.end_date = df['start_time'].values[-1]
        self.type_df = type_df

        df['is_maturity'] = df['start_time'].apply(lambda x: is_same_week_as_third_thursday(x))

        self.analyzer = InforIndicator(df, 
                                       name=f"{self.type_df} Total Analysis")
        self.analyzer.auto_analyze()

        if self.maturity:
            maturity_df = df[df['is_maturity'] == True]
            self.maturity_analyzer = InforIndicator(maturity_df, 
                                                    name=f"{self.type_df} Maturity Only")
            self.maturity_analyzer.auto_analyze()
            
            notmaturity_df = df[df['is_maturity']==False]
            self.nonmaturity_analyzer = InforIndicator(data=notmaturity_df, 
                                                name=f"{self.type_df} Except Maturity")
            self.nonmaturity_analyzer.auto_analyze()
        return vars(self)
        
    def run_from_csv(self, csv_file, type_df='train'):
        df = load_csv(csv_file)
        return self.run_from_df(df, type_df=type_df)
    ...
    def save_visualize_chart(self, save_dir):
        ...
        '''
        self.analyzer.save_chart(save_dir)
        self.maturity_analyzer.save_chart(save_dir)
        self.nonmaturity_analyzer.save_chart(save_dir)